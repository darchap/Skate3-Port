...
// Skate 3 occasionally reaches optional content / presentation callbacks
// with a stale guest object on Android. The bad dispatch values seen in the
// field are object sentinels or float data, not addresses of omitted recompiled
// functions. Each call site has an existing null/default path, so initialize
// its out parameter and follow that path instead of terminating the process.
//
// Keep this keyed to the exact return addresses. An unknown indirect call must
// still trap so genuine missing recompilation and memory corruption remain
// visible in diagnostics.
bool RecoverKnownSkate3InvalidCallback(PPCContext& ctx, uint8_t* base) {
  const uint32_t original_r3 = static_cast<uint32_t>(ctx.r3.u64);
  const uint32_t original_r4 = static_cast<uint32_t>(ctx.r4.u64);
  uint32_t output_address = 0;
  size_t output_size = 0;

  switch (static_cast<uint32_t>(ctx.lr)) {
    case 0x82B326C4:
      // Factory call. The caller immediately branches to its normal failure
      // return when r3 is null.
      ctx.r3.u64 = 0;
      break;
    case 0x82B3CD64:
      // Content-size callback: r4 points at a four-byte alignment result.
      output_address = static_cast<uint32_t>(ctx.r4.u64);
      output_size = 4;
      ctx.r3.u64 = 0;
      break;
    case 0x82B3CE2C:
      // Optional content-validation callback. The caller immediately checks
      // the low byte of r3 and follows its normal cleanup path when false.
      ctx.r3.u64 = 0;
      break;
    case 0x82B3C95C:
      // Optional cleanup notification stored at object +12. The destructor
      // checks the slot for null before calling it, but field reports show an
      // already-released object retaining 0xFFFDFFFF here. The callback has
      // no return value and the next instruction continues cleanup, so
      // skipping only this exact stale dispatch preserves the retail path.
      break;
    case 0x82B328D4:
      // Optional renderer/content notification. The return value is unused
      // and the caller returns immediately afterward. Field diagnostics from
      // Snapdragon devices show a released presentation object in r3 here,
      // producing a non-code vtable target during the FMV transition.
      break;
    case 0x82F2D074:
      // Streaming-buffer notification reached through sub_82F2BD00's tail
      // call. Its result is unused by sub_82F2CFB8, which continues with the
      // already-computed buffer pointer. AYN diagnostics show the optional
      // callback slot retaining the retail 0xFFFDFFFF sentinel.
      break;
    case 0x82805478:
      // Optional presentation callback reached while leaving a skipped front-
      // end movie. The callback result is unused and the caller immediately
      // continues its presentation sequence. An RG406V public-package cold
      // boot exposed a released object's non-code vtable target here.
      break;
    case 0x828DF620:
      // Per-item presentation notification at vtable +48. The caller does not
      // consume a return value and advances directly to the next list node.
      // A true first-run RG406V movie skip exposed the stale optional slot.
      break;
    case 0x82958D0C:
      // Optional presentation service query at vtable +16. The caller starts
      // with a null result and stores r3 as the new global service pointer.
      // Preserve the caller's existing no-service path for a stale object.
      ctx.r3.u64 = 0;
      break;
    case 0x82E9495C:
      // Presentation transform callback: r3 points at a four-float result.
      output_address = static_cast<uint32_t>(ctx.r3.u64);
      output_size = 16;
      break;
    default:
      return false;
  ...
  if (output_size != 0) {
    // Both outputs live on the active guest stack. Reject wraparound and an
    // unexpected pointer before touching the mapped guest address space.
    const uint32_t stack_pointer = static_cast<uint32_t>(ctx.r1.u64);
    if (output_address < stack_pointer || output_address - stack_pointer > 0x1000 ||
        output_address > UINT32_MAX - output_size) {
      return false;
    }
    std::memset(base + output_address, 0, output_size);
  }

  static std::atomic<int32_t> recovery_log_budget{24};
  if (recovery_log_budget.fetch_sub(1, std::memory_order_relaxed) > 0) {
    REXLOG_WARN(
        "Recovered invalid Skate 3 callback: target=0x{:08X} guest_lr=0x{:08X} "
        "r3=0x{:08X} r4=0x{:08X}",
        ctx.last_indirect_target, static_cast<uint32_t>(ctx.lr), original_r3, original_r4);
  }
  return true;
}
#endif
...
static void InvalidFunctionTrap(PPCContext& ctx, uint8_t* base) {
  ...
  const uint32_t thread_id = thread_state ? thread_state->thread_id() : 0;
#if REX_PLATFORM_ANDROID
  if (RecoverKnownSkate3InvalidCallback(ctx, base)) {
    return;
  }
  // Some retail Xbox 360 titles leave optional virtual callback slots null.
  // Native PPC execution effectively has no useful work to perform for such a
  // slot, while aborting the whole Android process here makes an otherwise
  // recoverable content callback fatal. Keep non-null unknown targets fatal so
  // missing recomp functions and corrupted vtables are still caught.
  if (ctx.last_indirect_target == 0) {
    static std::atomic<uint32_t> null_call_log_budget{8};
    if (null_call_log_budget.fetch_sub(1, std::memory_order_relaxed) > 0) {
      REXLOG_WARN(
          "Skipped null guest callback: guest_lr=0x{:08X} guest_tid=0x{:08X} "
          "r3=0x{:08X}",
          static_cast<uint32_t>(ctx.lr), thread_id, static_cast<uint32_t>(ctx.r3.u64));
    }
    return;
  }
#endif
  REX_FATAL(
      "Call to invalid or unregistered function: target=0x{:08X} guest_lr=0x{:08X} "
      "guest_sp=0x{:08X} guest_tid=0x{:08X} r3=0x{:08X} r4=0x{:08X} "
      "r5=0x{:08X} r6=0x{:08X} r7=0x{:08X} r8=0x{:08X}",
      ctx.last_indirect_target, static_cast<uint32_t>(ctx.lr), static_cast<uint32_t>(ctx.r1.u64),
      thread_id, static_cast<uint32_t>(ctx.r3.u64), static_cast<uint32_t>(ctx.r4.u64),
      static_cast<uint32_t>(ctx.r5.u64), static_cast<uint32_t>(ctx.r6.u64),
      static_cast<uint32_t>(ctx.r7.u64), static_cast<uint32_t>(ctx.r8.u64));
}
