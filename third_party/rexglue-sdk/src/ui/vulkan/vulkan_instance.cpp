...
#if REX_PLATFORM_ANDROID && defined(REX_HAS_ADRENOTOOLS)
  const char* custom_driver_dir = std::getenv("SKATE3_VULKAN_DRIVER_DIR");
  const char* custom_driver_name = std::getenv("SKATE3_VULKAN_DRIVER_NAME");
  const char* native_library_dir = std::getenv("SKATE3_NATIVE_LIBRARY_DIR");
  const char* internal_files_dir = std::getenv("SKATE3_INTERNAL_FILES_DIR");
