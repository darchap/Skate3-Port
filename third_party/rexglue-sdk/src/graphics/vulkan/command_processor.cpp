...
bool VulkanCommandProcessor::SetupContext() {
  ...
  if (!device_properties.vertexPipelineStoresAndAtomics) {
    ...
    // Mobile Mali parts may omit vertex-stage storage writes. The Android
    // Skate 3 port replaces scene rendering with the native renderer, and the
    // translator already has a compute-stage fallback for memory export.
    REXGPU_WARN(
        "Vulkan vertexPipelineStoresAndAtomics is unavailable; continuing "
        "with the Android native-renderer compatibility path");
#else
    REXGPU_ERROR(
...
void VulkanCommandProcessor::IssueSwap(uint32_t frontbuffer_ptr, uint32_t frontbuffer_width,
  ...
  // Native aspect override: while the native renderer is serving frames,
  // allocate the guest output at its requested aspect; emulated frames keep
  // the frontbuffer aspect and use the presenter's letterbox path.
  const bool native_aspect_output = ApplyNativeGuestOutputAspect(
  ...
  presenter->RefreshGuestOutput(
      ...
      [this, guest_output_width, guest_output_height, display_width, display_height,
       ...
       native_aspect_output](ui::Presenter::GuestOutputRefreshContext& context) -> bool {
        // In case the swap command is the only one in the frame.
        if (!BeginSubmission(true)) {
          return false;
        }
        ...
        if (HasNativeGuestOutputRenderer()) {
          ...
          // buffer.
          NativeRhiEndFrame(native_rhi_device_);
          if (native_aspect_output) {
            // The output was sized for the native renderer's aspect; the
