...
bool VulkanSharedMemory::Initialize() {
  ...
  bool allow_sparse_shared_memory = REXCVAR_GET(vulkan_sparse_shared_memory);
#if REX_PLATFORM_ANDROID
  // Qualcomm's proprietary Android drivers expose sparse buffer residency on
  // recent Adreno parts, but affected Adreno 730, 740, and 830 devices abort
  // while the game process is entering the Vulkan runtime. The Mali-G57 test
  // device does not take this path. Use the existing fully bound 512 MB
  // fallback on Qualcomm until sparse residency has passed device testing.
  constexpr uint32_t kQualcommVendorId = 0x5143;
  if (vulkan_device->properties().vendorID == kQualcommVendorId &&
      allow_sparse_shared_memory) {
    allow_sparse_shared_memory = false;
    REXGPU_WARN(
        "Shared memory: Disabling sparse Vulkan residency for the Android "
        "Qualcomm compatibility path");
  }
#endif
  ...
  if (allow_sparse_shared_memory && vulkan_device->properties().sparseResidencyBuffer) {
