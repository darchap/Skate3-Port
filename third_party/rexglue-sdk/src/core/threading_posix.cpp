...
class PosixConditionBase {
 public:
  ...
  static std::pair<WaitResult, size_t> WaitMultiple(std::vector<PosixConditionBase*>&& handles,
    ...
    // POSIX has no direct WaitForMultipleObjects equivalent.  All condition
    // types publish state changes through the shared epoch/CV below, allowing
    // a multi-wait to sleep until any member changes instead of polling every
    // millisecond.  Lock the member conditions in a stable address order so
    // concurrent multi-waits cannot deadlock when passed different orders.
    std::vector<PosixConditionBase*> lock_order = handles;
    std::sort(lock_order.begin(), lock_order.end(), std::less<PosixConditionBase*>());
    lock_order.erase(std::unique(lock_order.begin(), lock_order.end()), lock_order.end());
    std::vector<std::unique_lock<std::mutex>> locks;
    locks.reserve(lock_order.size());
    while (true) {
      const uint64_t observed_epoch = multi_wait_epoch_.load(std::memory_order_acquire);
      size_t first_signaled = std::numeric_limits<size_t>::max();
      bool condition_met = false;

      locks.clear();

      for (PosixConditionBase* handle : lock_order) {
#if REX_PLATFORM_LINUX && !REX_PLATFORM_ANDROID
      ...
      std::unique_lock<std::mutex> wait_lock(multi_wait_mutex_);
      const auto changed = [observed_epoch] {
        return multi_wait_epoch_.load(std::memory_order_acquire) != observed_epoch;
      };
      if (changed()) {
        continue;
      }
      if (end_time == std::chrono::steady_clock::time_point::max()) {
        multi_wait_cond_.wait(wait_lock, changed);
      } else if (!multi_wait_cond_.wait_until(wait_lock, end_time, changed)) {
        return std::make_pair<WaitResult, size_t>(WaitResult::kTimeout, 0);
      }
    }
  }
 ...
 protected:
  static void NotifyMultiWaiters() {
    multi_wait_epoch_.fetch_add(1, std::memory_order_release);
 ...
 private:
  inline static std::atomic<uint64_t> multi_wait_epoch_{0};
...
#if REX_PLATFORM_ANDROID
// Multi-waits wake immediately through PosixConditionBase's shared notifier.
// This slice only bounds delivery latency for callbacks that arrive through a
// signal rather than a waited handle; 1 ms made idle audio waits consume most
// of a CPU core on handheld SoCs.
constexpr auto kAlertablePollSlice = std::chrono::milliseconds(8);
#else
...
void* PosixCondition<Thread>::ThreadStartRoutine(void* parameter) {
  ...
  {
    // Resume() waits only for the state but gates on the count, so both must be
    // published under this lock: a create-suspended thread resumed into nothing
    // waits forever.
