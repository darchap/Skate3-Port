...
#if REX_PLATFORM_ANDROID
  // The app targets API 33+, where memfd_create is available. Prefer it over
  // ASharedMemory for the 4.5 GB sparse backing object used by the Xbox 360
  // guest address space. Some Android 16 and 17 devices reject that size via
  // ASharedMemory_create even though an ordinary sparse memfd is valid.
  int sharedmem_fd = memfd_create(path.c_str(), MFD_CLOEXEC | MFD_ALLOW_SEALING);
  if (sharedmem_fd >= 0) {
    if (ftruncate64(sharedmem_fd, static_cast<off_t>(length)) == 0) {
      // The guest mapping size is immutable after setup. Sealing only size
      // changes leaves all required shared read/write mappings available.
      (void)fcntl(sharedmem_fd, F_ADD_SEALS, F_SEAL_GROW | F_SEAL_SHRINK);
      return static_cast<FileMappingHandle>(sharedmem_fd);
    }
    close(sharedmem_fd);
  }

  // Retain the platform API as a fallback for unusual vendor kernels.
  if (android_ASharedMemory_create_) {
    sharedmem_fd = android_ASharedMemory_create_(path.c_str(), length);
