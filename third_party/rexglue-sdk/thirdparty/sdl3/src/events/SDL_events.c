                                  ...
                                  Uint32 minType, Uint32 maxType, bool include_sentinel)
{
    ...
    if (used > 0 && action == SDL_ADDEVENT) {
        ...
        // The poll sentinel is inserted by SDL_PumpEventsInternal on the
        // thread that is about to wait. On Android, waking the lifecycle
        // semaphore for this private bookkeeping event makes SDL_WaitEvent
        // immediately pump another sentinel and wake itself again, turning
        // the otherwise-idle UI thread into a busy loop. Real queued events
        // must still wake the thread immediately.
        for (i = 0; i < numevents; ++i) {
            if (events[i].type != SDL_EVENT_POLL_SENTINEL) {
                needs_wakeup = true;
                break;
            }
        }
